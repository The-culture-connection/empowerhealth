...
- **Screen**: **`CareNavigationSurveyScreen`** — `lib/care_survey/care_navigation_survey_screen.dart` (`Routes.careSurvey`). Intro copy matches **NewUI** (“Let’s check in on your care”); privacy card **This is just for you**. Completion thanks the user (**Thank you for trusting us**) with a **warm next-step** message (explore app resources at your own pace)—**no** prompt to call a care team. **`Skip to finish`** awaits Firestore save when no needs are selected.
...
### Current Functionality
The **After-Visit Support** flow (user-facing name; same technical feature as visit summarizing) lets users **take a photo** of paperwork or **choose a PDF**, or **type notes**, from appointments. AI simplifies language—not diagnoses. A **one-time transparency dialog** (`shared_preferences` key `after_visit_transparency_v1`) explains that the tool does not replace the provider. Output is shown in section cards **In Simpler Words**, **Important Information**, and **Questions You May Want to Ask** when the summary contains multiple `##` markdown sections; otherwise a single **In Simpler Words** card is used. Summaries stay aimed at accessible reading level where the profile supplies it.

### How the feature works
- **Visits list (Flutter)**: **`AppointmentsListScreen`** — `lib/appointments/appointments_list_screen.dart`. Title **My Visits** with affirming subtitle and reassurance card (aligned with **NewUI `MyVisits.tsx`**), then **Past visits**. Summaries use `users/{uid}/visit_summaries` ordered by **`createdAt`** (newest first), with duplicate rows for the same calendar day merged. Tapping a row opens **`VisitDetailScreen`** — `lib/appointments/visit_detail_screen.dart` — a **full-page** visit detail (aligned with **NewUI `VisitDetail.tsx`**, not a modal): sections include About, What was discussed, Actions to take, **Questions to ask next time**, **Notes** (from AI `visitNotes`), suggested learning, and a reminder.
